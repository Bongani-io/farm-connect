...
export default function CustomDataTable(){
    ...
    const currentDispledData = data.slice(startIndex, endIndex);
  ...
  return (
    <div className=''>
        <h2 className='text-xl font-bold mt-5 ml-8'>Recent Orders</h2>
            ...
            
    {
        Array.from({ length: totalPages }, (_, index) => {
            return (
                <li key={index}>
                    <button onClick={()=>setCurrentPage(index+1)} disabled={currentPage==index+1} className={currentPage==index+1 ? "flex items-center justify-center px-3 h-8 leading-tight text-blue-600 bg-blue-100 border border-blue-300 hover:bg-blue-200 hover:text-blue-700 dark:bg-gray-700 dark:border-gray-600 dark:text-white" : "flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white"}>
                        {index + 1}
                    </button>
                </li>
            );
